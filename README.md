...
音乐卡片右上角「更换」可选择本机已安装的 `.app` 音乐软件，选择保存在本机配置中，重启后保留；首次默认汽水音乐。汽水音乐支持播放 / 暂停 / 切歌（状态由面板操作估算）；Apple Music 和 Spotify 使用应用脚本接口读取播放状态与控制，可能需要系统自动化授权。其他客户端暂只支持打开，隐藏切歌按钮；更换不会停止原软件正在播放的音乐。
...
Windows 首版暂不显示「当前窗口」和「音乐」组件；剪贴板点击复制后用 Ctrl+V 粘贴，AI 完成提醒暂不支持点击切回任务窗口。平台限制不覆盖原有组件偏好。镜子仅主动点击后开启，离开首页或收起立即释放；结束录音释放麦克风。系统拒绝访问设备时，请检查 Windows 隐私设置中桌面应用的相机 / 麦克风权限。
