...
- 首页新增「现在模式」：聚焦下一条待办、专注计时、后续事项与本机 AI 完成事件；可随时切回原 Bento 工作台，且会记住视图偏好。
- 本机 AI 完成事件新增「结果收件箱」：可在工作区中归档、恢复，或预填为一条跟进待办。
- 智能命名与分组支持智谱 GLM、MiniMax 及其他 OpenAI-compatible 服务；服务商预设会填入对应的官方 Base URL 与默认模型，API Key 继续仅由系统安全存储保存。
